...
public:
	TreeNode* lca(TreeNode* root){
		...
		unordered_map<TreeNode*, TreeNode*> parent; 
		q.push(root); 
		unordered_set<TreeNode*> lastlvl; 
		...
		if (lastlvl.size() == 1)
			return *lastlvl.begin(); 
		
		unordered_set<TreeNode*> p; 
...
public:
	...
	int lca(TreeNode* node, TreeNode *& common, int depth){
		if (!node){
			common = nullptr; 
			return depth; 
		}
		...
		TreeNode *lnxt, *rnxt; 
		int ldep = lca(node->left, lnxt, depth + 1),
			rdep = lca(node->right, rnxt, depth + 1); 
...
public:
	...
	int dfs(TreeNode *n, int dep, int& maxd, TreeNode *& res){
		if (!n) return dep - 1; 
		int ldep = dfs(n->left, dep + 1, maxd, res),
			rdep = dfs(n->right, dep + 1, maxd, res); 
